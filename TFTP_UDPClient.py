...
def write(sock,ip,port,*args,**kwargs):
    ...
    recieved=False
    sock.sendto(struct.pack(f"!H{len(args[0])}sB{len('netascii')}sB",2,args[0].encode(),0,b'netascii',0), (ip,int(port)) ) 
...
def read(sock,ip,port,*args,**kwargs):
    ...
    estructura=struct.pack(f"!H{len(args[0])}sB{len('netascii')}sB",1,str.encode(args[0]),0,b'netascii',0)
            ...
            try:
                ...
                if not acknowledgment:
                    ...
                    if(unpacked_code[0]==3):
                        ...
                        f.write(unpacked_data[1].decode("utf-8"))
                        
                        #Enviamos codigo 4 de ACK y el numero de bloque al que corresponde el acknowledgment
                        sock.sendto(struct.pack('!2H', 4 , unpacked_data[0]) , cliente )   #ACK
...
def main(*args,**kwargs):
    ...
    while True:
        try:
            ...
            functions[arguments[0]](sock,sys.argv[1],sys.argv[2],arguments[1])
        except Exception:
            traceback.print_exc()
        finally:
            sock.close()
